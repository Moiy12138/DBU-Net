...
class KANLinear(torch.nn.Module):
    ...
    def b_splines(self, x:torch.Tensor):
        """
        Compute the B-spline bases for the giveni input tensor.
        ...
        grid = torch.concatenate(
            [
                ...
                * torch.arange(self.spline_order, 0, -1, device=x.device).unsqueeze(1),
                ...
                grid[-1:]
                + uniform_step
                * torch.arange(1, self.spline_order, device=x.device).unsqueeze(1),
            ],
            dim=0,
        )
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ...
        regulariztion_loss_entropy = - torch.sum(p * p.log())

        return (
            regularize_activation * regularize_loss_activation
            + regularize_entropy * regulariztion_loss_entropy
        )
